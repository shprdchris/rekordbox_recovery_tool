...
class rbrDatabase:
    def __init__(self):
        ...
        # Access nodes via their path: can access any parent nodes if you know the child's path
    ...
    def print_warning_message(self, specific_submessage, file_name):
        print("WARNING: ", specific_submessage," ", file_name,
            ", not supported by pre-2000 model CDJs")

    # ------------------------------------------------------------------------------

    def added_track_id(self, abs_track_path, file_name, file_extension, show_warning_flags):
        ...
        self.collection.set('Entries', track_temp.metadata['TrackID']) #TODO inefficient to do every time
        ...
        if not os.path.isdir(root_dir):
            ...
            for root, dirs, files in os.walk(root_dir):
                ...
                if self.dict_paths_nodes == {}:
                    current_element = self.added_folder(self.playlists, 'ROOT', str(len(dirs)))
                    self.dict_paths_nodes[root] = current_element
                elif dirs:
                    # Folder of playlists/folders
                    parent_element = self.dict_paths_nodes[dict_key]
                    current_element = self.added_folder(parent_element, node_name, str(len(dirs)))
                    self.dict_paths_nodes[root] = current_element
                elif not dirs:
                    # Playlist: add all tracks to collection and add their keys to the playlist node
                    parent_element = self.dict_paths_nodes[dict_key]
                    files = self.files_surviving(files)
                    current_element = self.added_playlist(parent_element, node_name, str(len(files)))
                    self.dict_paths_nodes[root] = current_element
                    # Add all audio files to collections
                    for file in files:
                        file_path = os.path.join(root, file)
                        split_file = file.split('.')
                        current_id =self.added_track_id(file_path, split_file[len(split_file)-2], split_file[len(split_file)-1],show_warning_flags)
