...
class gui:
    ...
    def create_db_and_print(self, file_name_user, root_folder, out_folder, show_warning):
    ...
    def __init__(self):
        ...
        check_warn = tk.Checkbutton(self.root_tk, text = "Print compatibility warnings to console", variable = self.show_warning)
        check_warn.pack()

        # "Run" button
        run_text = tk.StringVar()
        run_text.set ("RUN")
        run_button = tk.Button(self.root_tk, textvariable = run_text, command = lambda:self.run(self.entry_db_name, self.show_warning.get() ))
        run_button.pack()

        # "Close" button
        close_text = tk.StringVar()
        close_text.set ("Close")
        close_button = tk.Button(self.root_tk, textvariable = close_text, command = lambda:self.root_tk.destroy())
        close_button.pack()
